...
loop do
	...
	loop do
		...
		action, action_cursor = prompt.select("#{header} | Node", action_opts, default: action_cursor, cycle: true, filter: true)
		system 'clear'
		...
		when :select
			select_cursor = 1
			loop do
				node, select_cursor = prompt.select("#{header} | Select", node_opts, default: select_cursor, cycle: true, filter: true, per_page: 10)
				system 'clear'
				...
				loop do
					...
					command, view_cursor = prompt.select('Continue?', command_opts, default: view_cursor, cycle: true, filter: true)
					system 'clear'
		...
		when :menu
			...
			command = prompt.select("#{header} | Menu", commands, cycle: true, filter: true)
			system 'clear'
